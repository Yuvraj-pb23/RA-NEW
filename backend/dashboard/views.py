...
class HORequiredMixin:
    def dispatch(self, request, *args, **kwargs):
        check_role(request.user, [SystemRole.SUPER_ADMIN, SystemRole.ORG_ADMIN, SystemRole.HO_USER])
        return super().dispatch(request, *args, **kwargs)

class RORequiredMixin:
    def dispatch(self, request, *args, **kwargs):
        check_role(request.user, [SystemRole.SUPER_ADMIN, SystemRole.ORG_ADMIN, SystemRole.HO_USER, SystemRole.RO_USER])
        return super().dispatch(request, *args, **kwargs)

class PIURequiredMixin:
    def dispatch(self, request, *args, **kwargs):
        check_role(request.user, [SystemRole.SUPER_ADMIN, SystemRole.ORG_ADMIN, SystemRole.HO_USER, SystemRole.RO_USER, SystemRole.PIU_USER])
        return super().dispatch(request, *args, **kwargs)
...
class DashboardHomeView(DashboardMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user_role in [SystemRole.SUPER_ADMIN, SystemRole.ORG_ADMIN, SystemRole.HO_USER]:
            cards.append({
                "label": "Users",
                "url":   "/api/v1/users/",
...
class OrgUnitListView(RORequiredMixin, DashboardMixin, TemplateView):
    template_name = "dashboard/org_units/list.html"
    active_page   = "org_units"

class ProjectListView(PIURequiredMixin, DashboardMixin, TemplateView):
...
class UserListView(HORequiredMixin, DashboardMixin, TemplateView):
