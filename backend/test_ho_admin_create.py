...
from orgs.models import OrgUnit, OrgLevel
...
org = Organization.objects.create(name="Forest Dept", acronym="FD")

ho_level = OrgLevel.objects.create(name="HO", level_number=1)
ro_level = OrgLevel.objects.create(name="RO", level_number=2, parent_level=ho_level)

# Create HO Unit directly
ho_unit = OrgUnit.objects.create(
    organization=org,
    level=ho_level,
    name="Head Office"
)
...
user = User.objects.create(username="super", role="SUPER_ADMIN", is_superuser=True)
request.user = user
...
other_ho_unit = OrgUnit.objects.create(
    organization=org,
    level=ho_level,
